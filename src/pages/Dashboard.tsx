  ...
  return (
    <div className="mx-auto grid w-full max-w-6xl gap-6 lg:grid-cols-[1.08fr_0.92fr]">
      ...
      <Card className="border-white/10 bg-white/[0.03] shadow-2xl shadow-black/20">
        <CardHeader><div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-2xl bg-violet-300/10 text-violet-200"><Users className="size-5" /></div><div><CardTitle className="text-xl text-white">Join your crew</CardTitle><p className="mt-1 text-sm text-slate-500">Already have a room code? Drop in.</p></div></div></CardHeader>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#0c1324]/70 p-5 lg:col-span-2"><div className="flex flex-wrap items-center gap-x-8 gap-y-3 text-xs text-slate-500"><span className="inline-flex items-center gap-2"><Swords className="size-4 text-rose-300" /> Send clockwise</span><span className="inline-flex items-center gap-2"><Shield className="size-4 text-cyan-300" /> Defend your gate</span><span className="inline-flex items-center gap-2"><RotateCcw className="size-4 text-amber-300" /> Resolve each wave</span><span className="ml-auto font-mono text-[10px] tracking-[0.2em] text-slate-700">NO EXTRA MODES · V1</span></div></div>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.82fr_1.38fr_0.82fr]">
        ...
        <Card className="border-white/10 bg-[#0f1729] shadow-xl shadow-black/20"><CardHeader className="border-b border-white/10 pb-5"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] tracking-[0.2em] text-slate-600">THE RING</p><CardTitle className="mt-2 text-xl text-white">Pass-along map</CardTitle></div><div className="flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-xs text-amber-200"><Activity className="size-3.5" /> {healthTotal} integrity</div></div></CardHeader><CardContent className="space-y-3 pt-5">{room.players.map((seat, index) => <div key={String(seat.userId)}><PlayerSeat player={seat} index={index} isCurrent={index === currentIndex} isHost={index === 0} total={room.players.length} />{index < room.players.length - 1 && <div className="flex h-6 items-center justify-center gap-2 text-[10px] font-mono uppercase tracking-[0.18em] text-slate-700"><ArrowDownToLine className="size-3.5" /> next gate receives the pass</div>}</div>)}<div className="rounded-xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-3 text-center text-xs leading-5 text-slate-600">Your send always lands on the station immediately below you. Cover the player who covers you.</div></CardContent></Card>

        <Card className={`border-rose-300/25 shadow-[0_0_35px_rgba(251,113,133,0.05)] ${player.incoming > 0 ? "bg-rose-300/[0.08]" : "bg-white/[0.03]"}`}><CardHeader><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-white"><Shield className="size-4 text-rose-200" /> At your gate</div><span className={`rounded-full px-2 py-1 font-mono text-[10px] ${player.incoming > 0 ? "bg-rose-300/15 text-rose-200" : "bg-white/5 text-slate-600"}`}>{player.incoming} INCOMING</span></div></CardHeader><CardContent className="space-y-6"><div className="rounded-2xl border border-rose-300/15 bg-black/15 p-5 text-center"><p className="font-mono text-5xl text-rose-100">{String(player.incoming).padStart(2, "0")}</p><p className="mt-2 text-xs text-slate-500">unresolved units</p></div><div><p className="text-sm font-semibold text-white">Defend the gate</p><p className="mt-1 text-xs leading-5 text-slate-500">Spend shield charge to intercept before the wave resolves.</p><div className="mt-4 flex gap-2">{[1, 3, 5].map((amount) => <button type="button" key={amount} onClick={() => setDefendAmount(amount)} className={`flex-1 rounded-lg border py-2 font-mono text-sm transition ${defendAmount === amount ? "border-rose-300/40 bg-rose-300/15 text-rose-100" : "border-white/10 bg-white/[0.03] text-slate-500 hover:text-white"}`}>{amount}</button>)}</div><Button type="button" onClick={() => onDefend(defendAmount)} disabled={isBusy || !canDefend || defendAmount > player.incoming || defendAmount > player.shield} className="mt-3 h-11 w-full bg-rose-300 text-slate-950 hover:bg-rose-200"><Shield className="size-4" /> Intercept {defendAmount}</Button></div><div className="border-t border-white/10 pt-4"><div className="flex items-center gap-2 text-xs text-slate-500"><HeartPulse className="size-3.5 text-rose-300" /> Unresolved units deal 7 damage each.</div></div></CardContent></Card>
      </div>

      <div className="grid gap-5 lg:grid-cols-[1fr_auto]"><div className="rounded-2xl border border-white/10 bg-[#0f1729]/80 px-5 py-4"><div className="flex items-start gap-3"><div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.05] text-slate-400"><Radio className="size-4" /></div><div><p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-600">relay log</p><p className="mt-1 text-sm text-slate-300">{room.lastAction}</p></div></div></div><div className="flex flex-col gap-3 rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] p-4 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><div className="flex size-9 items-center justify-center rounded-xl bg-amber-300/10 text-amber-200"><RotateCcw className="size-4" /></div><div><p className="text-sm font-semibold text-white">Resolve wave {room.wave}</p><p className="mt-0.5 text-[11px] text-slate-500">Host only · +4 units after</p></div></div>{isHost && <Button type="button" onClick={onResolve} disabled={isBusy} className="h-10 bg-amber-300 text-slate-950 hover:bg-amber-200">Resolve <ArrowRight className="size-4" /></Button>}</div></div>
      {error && <p className="rounded-xl border border-rose-300/20 bg-rose-300/10 px-4 py-3 text-sm text-rose-200">{error}</p>}
      <div className="flex flex-wrap items-center justify-between gap-3 px-1 text-[10px] uppercase tracking-[0.18em] text-slate-700"><span>Send clockwise · defend incoming · resolve wave</span><span className="inline-flex items-center gap-1.5"><Minus className="size-3" /> No extra modes in v1</span></div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const handleJoin = () => run(async () => {
    ...
    toast.success("You joined the relay.");
  });
